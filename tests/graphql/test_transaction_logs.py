...
"""
result_committable = {"data": {"total": 1, "committable": 0, "not_committable": 1}}
...
"""
result_async_fields = json.loads(
    json.dumps(
        {
            "data": {
                "tx_log": [
...
def test_transaction_logs_queries(
    ...
    with pg_driver_clean.session_scope() as session:
        ...
        session.merge(
            models.submission.TransactionLog(
                is_dry_run=True,
                program="CGCI",
                project="BLGSP",
                role="create",
...
def test_transaction_log_related_cases(client, submitter, pg_driver_clean, cgci_blgsp):
    utils.reset_transactions(pg_driver_clean)
    post_example_entities_together(client, pg_driver_clean, submitter)
    ...
    print(r.data)
    related_case = r.json["data"]["a"][0]["documents"][0]["response"]["entities"][1][
        "related_cases"
    ][0]
...
def test_transaction_logs_quick_search(client, submitter, pg_driver_clean, cgci_blgsp):
    utils.reset_transactions(pg_driver_clean)
    post_example_entities_together(client, pg_driver_clean, submitter)
    with pg_driver_clean.session_scope():
