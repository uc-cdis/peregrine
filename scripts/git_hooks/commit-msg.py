...
def header_check(real_lineno, line):
    ...
    if not m or len(m.groups()) != 3:
        return "Line %d: Header does not follow format: type(scope): message" % (
            real_lineno,
        )
    ...
    if len(line) > 50:
        return (
            "Line %d: First line should be less than 50 characters in length. (is %d)"
            % (real_lineno, len(line))
        )
...
def check_format_rules(lineno, line):
    ...
    if not line.startswith("#") and len(line) > 72:
        return "Line %d: No line should be over 72 characters long. (is %d)" % (
            real_lineno,
            len(line),
        )
